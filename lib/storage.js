...
/**
 * Applies `updater` and persists immediately.
 *
 * This deliberately does not debounce: the write runs in an MV3 service
 * worker, which Chrome may terminate as soon as the popup closes, so a
 * deferred save can be dropped and the user's edit lost. The AES key is
 * cached after first use, making each write sub-millisecond at this size.
 */
...
export { getState, setState, makeProfile, migrate, uid, save };
