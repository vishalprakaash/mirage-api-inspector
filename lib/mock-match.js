...
(function (global) {
  ...
  /**
   * Absolutises a request URL against the page.
   *
   * Both fetch() and XHR accept relative URLs ('/api/users'), but every url
   * pattern is written against a full URL, so matching must happen on the
   * resolved form or nothing with a filter would ever match.
   */
  function resolveUrl(url, base) {
    ...
    try {
      return new URL(raw, fallback).href;
    } catch {
      return raw;
    }
  }
